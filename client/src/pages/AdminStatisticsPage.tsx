...
// Interfaces para los datos estadísticos (mismo formato que en StatisticsPage)
...
const AdminStatisticsPage = () => {
  ...
  const [stats, setStats] = useState<AdminStatisticsData>({
    appointmentsByMonth: [],
    appointmentsByType: [],
    ...
    const fetchStatistics = async () => {
      ...
      try {
        ...
        if (!user || user.userType !== 'admin') {
          setError("Acceso no autorizado. Se requieren permisos de administrador.");
          setIsLoading(false);
          return;
        }
        
        // Obtener datos de administrador desde la API
        const response = await apiRequest("GET", "/api/statistics/admin");
        
        if (!response.ok) {
          if (response.status === 403) {
            setError("Acceso no autorizado. Se requieren permisos de administrador.");
          } else {
            setError("Error al cargar las estadísticas: " + response.statusText);
          }
          setIsLoading(false);
          return;
        }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-screen">
        ...
        <p>Cargando estadísticas de administrador...</p>
      </div>
    );
  }
    ...
    return (
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="text-center">
          <p className="mb-4">No tienes permiso para ver esta página o ha ocurrido un error.</p>
          <a href="/" className="text-equi-green hover:underline">Volver al inicio</a>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-equi-charcoal mb-2">Panel de Administración - Estadísticas Globales</h1>
        <p className="text-gray-600">
          Visualiza los datos globales de la plataforma EquiGest
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        ...
        <Card className="bg-white border-equi-brown/20 hover:shadow-md transition-shadow">
          <CardContent className="pt-6">
            <div className="flex items-center">
        ...
        <Card className="bg-white border-equi-green/20 hover:shadow-md transition-shadow">
          <CardContent className="pt-6">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-gray-500 text-sm">Profesionales</p>
                <h3 className="text-2xl font-bold">{stats.totalProfessionals}</h3>
              </div>
            </div>
          </CardContent>
        </Card>

        <Card className="bg-white border-equi-gold/20 hover:shadow-md transition-shadow">
          <CardContent className="pt-6">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-gray-500 text-sm">Caballos registrados</p>
        ...
        <Card className="bg-white border-equi-charcoal/20 hover:shadow-md transition-shadow">
          <CardContent className="pt-6">
            <div className="flex items-center">
              <div className="bg-equi-charcoal/10 p-3 rounded-full mr-4">
                <TrendingUp className="h-6 w-6 text-equi-charcoal" />
              </div>
              <div>
        ...
        <TabsContent value="services" className="mt-0">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Porcentaje de cada tipo de servicio profesional en la plataforma
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-80">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Número de citas por cada profesional en la plataforma
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-72">
